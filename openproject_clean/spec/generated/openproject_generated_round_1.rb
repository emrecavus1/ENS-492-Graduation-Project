...
RSpec.describe "Generated OpenProject Request Coverage", :skip_csrf, type: :rails_request do
  ...
  describe "api_v3_projects" do
    context "with current_user" do
      ...
      it "GET /api/v3/projects with pageSize and offset params returns a valid response" do
        get "/api/v3/projects", params: { pageSize: 1, offset: 0 }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end

      it "GET /api/v3/projects with sortBy param returns a valid response" do
  ...
  describe "api_v3_work_packages" do
    context "with current_user" do
      ...
      it "GET /api/v3/work_packages with pageSize and offset params returns a valid response" do
        get "/api/v3/work_packages", params: { pageSize: 1, offset: 0 }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end

      it "GET /api/v3/work_packages with sortBy param returns a valid response" do
        get "/api/v3/work_packages", params: { sortBy: '[["id","asc"]]' }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end

      it "GET /api/v3/work_packages with include param returns a valid response" do
        get "/api/v3/work_packages", params: { include: "project,status,assignee" }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end
      ...
      it "GET /api/v3/work_packages with enriched params returns a valid response" do
  ...
  describe "api_v3_users" do
    context "with current_user" do
      ...
      it "GET /api/v3/users/:id with sortBy param returns a valid response" do
        id = seed_user.id
        get "/api/v3/users/#{id}", params: { sortBy: '[["id","asc"]]' }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end

      it "GET /api/v3/users/:id with offset and pageSize params returns a valid response" do
        id = seed_user.id
        get "/api/v3/users/#{id}", params: { offset: 0, pageSize: 1 }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end

      it "GET /api/v3/users with enriched params returns a valid response" do
        get "/api/v3/users", params: { status: "active", pageSize: 1, offset: 0 }
        expect([200, 201, 301, 302, 401, 403, 404, 422]).to include(response.status)
      end
    end
